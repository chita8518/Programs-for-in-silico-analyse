...
scatter_df = pd.DataFrame({
    ...
    "mean_expr": expected_expr_mean.values   # теперь график по expected_expr
})


# === ПАРАМЕТРЫ ДЛЯ УДОБНОГО УПРАВЛЕНИЯ ===
...
for _, row in scatter_df.iterrows():
    ...
    # подпись чуть правее и выше точки
    t = ax.text(
        x + 0.01,
        y + 0.01,
        gene,
...
# линии от точки к подписи
lines = []
for t, (_, row) in zip(texts, scatter_df.iterrows()):
    x = row["tau"]
    y = row["mean_expr"]
    lx, ly = t.get_position()
    line = ax.plot([x, lx], [y, ly], color=LINE_COLOR, lw=0.5)[0]
    lines.append(line)

# adjustText разрулит пересечения
adjust_text(
    texts,
...
plt.xlabel("Tau specificity")
plt.ylabel("expected expression")
plt.title("Tau × expected expression")

plt.tight_layout()
plt.savefig(OUTPUT_DIR / "tau_vs_expression.png", dpi=300)
